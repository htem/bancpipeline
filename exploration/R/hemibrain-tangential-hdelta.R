...
### Data source paths
# Paths to feather files containing connectivity data
...
# hemibrain.meta.path <- "/n/data1/hms/neurobio/wilson/banc/connectivity/hemibrain_v.1.2.1_meta.feather" # On O2
# hemibrain.edgelist.path <- "/n/data1/hms/neurobio/wilson/banc/connectivity/hemibrain_v.1.2.1_edgelist.feather" # On O2
...
hb.meta <- arrow::read_feather(hemibrain.meta.path)

# Read the edgelist
hb.elist <- arrow::read_feather(hemibrain.edgelist.path)
...
aggregated_data <- hb.elist.collapsed %>%
  ...
  summarise(total_count = mean(count), .groups = "drop")
