...
fw.elist.merged <- fw.elist.th %>%
  left_join(fw.meta %>% select(root_783, cell_type, top_nt), by = c("pre" = "root_783")) %>%
  rename(pre_cell_type = cell_type, pre_top_nt = top_nt) %>%
  left_join(fw.meta %>% select(root_783, cell_type, top_nt), by = c("post" = "root_783")) %>%
  rename(post_cell_type = cell_type, post_top_nt = top_nt)

# Reshape and collapse the data by cell_type
fw.elist.collapsed <- fw.elist.merged %>%
...
for (conn_type in unique(fw.elist.collapsed$connection_type)) {
  filename <- file.path(fafb.images, paste0("heatmap_collapsed_", gsub(" ", "_", conn_type), ".png"))
  create_heatmap(fw.elist.collapsed, conn_type, filename)
}