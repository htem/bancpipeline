#' banc-manc-nblast-images — Render per-query BANC↔MANC NBLAST review PNGs.
#'
#' @section Reads:
#'   - per-query NBLAST CSVs under `<banc.nblast.manc.save.path>/results/<ver>`
#'   - BANC + MANC BANC-space meshes
#'
#' @section Writes:
#'   - `<banc.nblast.manc.save.path>/images/<ver>/todo/*.png`
#'
#' @section Invoked by:
...
# Get save folder
tracing.folder <- file.path("tracing","manc")
dir.create(tracing.folder, showWarnings = FALSE)
dir.results <- file.path(banc.nblast.manc.save.path,"results",version)
dir.images <- file.path(banc.nblast.manc.save.path,"images",version,"todo")
dir.create(dir.images, showWarnings = FALSE, recursive = TRUE)
dir.done <- file.path(banc.nblast.manc.save.path,"images",version,"done")
...
banc.meta <- banc.meta.all %>%
  dplyr::filter(is.na(manc_match),
                #is.na(cell_type)|grepl("auto",cell_type)|cell_type=="",
...
for (sp in super_classes) {
  ...
  banc_nblast_images(nblast.files = sample(nblast.files.chosen),
                     ...
                     other.obj.save.path = banc.nblast.manc.obj.save.path.version,
                     query.id = "root_id",
                     other.id = "bodyid",
                     numCores = 1,
                     query.mirror = FALSE,
...
# # Purge images for neurons that now have a manc_match
# done.images <- list.files(dir.images, recursive = TRUE, full.names = TRUE, pattern = "png")
# if (length(done.images)) {
#   matched.ids <- na.omit(unique(banc.meta.all$root_id[!is.na(banc.meta.all$manc_match)]))
