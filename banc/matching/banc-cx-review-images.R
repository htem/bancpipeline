#' banc-cx-review-images — Generate comparison PNGs for CX connectivity mismatches.
#'
#' For each row in `cx_connectivity_mismatches.csv`, render a three-mesh
#' comparison PNG: BANC neuron (blue), curated SeaTable-type FAFB neurons
#' (red), connectivity-matched FAFB neuron (green). Uses `fafb_match` where
#' available; falls back to all FAFB neurons of `fafb_cell_type` on the
#' same side.
#'
#' @section Reads:
...
#'
#' @section Writes:
#'   - `inst/images/cx_review/*.png` — per-mismatch comparison images

###########################################################
### CX connectivity match review images
###
### For each entry in cx_connectivity_mismatches.csv,
...
options(warn = 1)
message("### banc: CX connectivity match review images ###")
...
mismatch_csv <- "data/codex/cx_connectivity_mismatches.csv"
if (!file.exists(mismatch_csv)) {
  message("  Mismatch CSV not found: ", mismatch_csv)
  message("  Run banc-cx-connectivity-matches.R first.")
  return(invisible())
}
...
dir.images <- file.path(path.expand("~/Desktop"), "matching", "cx_review")
